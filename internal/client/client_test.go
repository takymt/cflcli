...
func TestListPages_QueryAndAuth(t *testing.T) {
	...
	result, err := cli.ListPages("SPACE-1", 25, "CURSOR-1")
	if err != nil {
...
func TestResolveSpaceIDByKey(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if got := r.URL.Query().Get("keys"); got != "WORK" {
				t.Fatalf("keys=%q", got)
			}
			w.Header().Set("Content-Type", "application/json")
		...
		spaceID, err := cli.ResolveSpaceIDByKey("WORK")
		if err != nil {
			t.Fatalf("ResolveSpaceIDByKey: %v", err)
		}
		if spaceID != "SPACE-1" {
			t.Fatalf("spaceID=%q want %q", spaceID, "SPACE-1")
		}
	})
	...
	t.Run("not found", func(t *testing.T) {
		...
		_, err = cli.ResolveSpaceIDByKey("WORK")
